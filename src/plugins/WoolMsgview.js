...
function fnCenterView(view) {
	if (view.classList.contains("loading") && view.querySelector(".wool_msgview_msg").textContent.length > 0) {
...
function MessageView(plainObject) {
	...
	var shadow = (this.shadow = wool.creatElement('div', {
		parent: body,
		classes: 'wool_msgview_modal'
	})); //控件
	var view = this.view =
		wool.creatElement('div', {
			parent: body,
			classes: 'wool_msgview info'
		});
	var loadingview = wool.creatElement('div', {
		parent: view,
		classes: 'wool_msgview_loading'
	});
	//外部获取消息内容控件
	this.msgView = wool.creatElement('div', {
		parent: view,
		classes: 'wool_msgview_msg'
	}); // 设置遮罩样式，并添加到页面
	if (!params.iswait) {
	...
	this.setMessage = function(message) {
		_this.msgView.innerHTML = wool.trim(message);
	};
	this.setMessage(params.msg);
	fnCenterView(view);
	if (params.icontype && (wool.getRealLength(params.msg) <= 36 ||
			params.iscustomlength)) {
		setIcon('iconable ' + params.icontype);
	}
	for (let callback of ONSHOWEVENTS) {
		callback(this);
	}
}
let beforeshowmsg = null;
export default {
	setOnShowmsgs(callback) {
		beforeshowmsg = callback;
	},
	showmsgs: function(opt, msg, delay, callback) {
		if (beforeshowmsg && beforeshowmsg() !== true) {
			return;
		}
