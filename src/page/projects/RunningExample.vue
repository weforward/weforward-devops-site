    ...
    <div class="table_area">
      <div class="query">
        <div class="query_input">
          <input ref="keywords" type="text" v-model="keywords" @keyup.enter='queryShiliList'/>
          <div class="query_button" @click="queryShiliList">
            查询
          </div>
        </div>
        ...
        <div class="query_group">
          ...
          <div class="add_button" @click="addrunning">
            +添加
          </div>
        </div>
      </div>
        ...
        <tr class='shili_items' v-for="(item,index) in runningList" :key='index'>
         <td>
           <span v-if="item.project" >
              ...
              <img :title="item.id+'[双击复制]'" v-if="item.project.type==1" src="../../img/java.png" class="javamark" @dblclick="copy(item.id)"/>
              <img :title="item.id+'[双击复制]'" v-if="item.project.type==2" src="../../img/html.png" class="htmlmark" @dblclick="copy(item.id)"/>
          ...
          <td>
            <span v-if="item.isloadstate"><Loading :isshow="item.isloadstate"></Loading></span>
            <span v-else :title="item.state_name" :class="[0==item.state ? 'stateblue' : 'statered']">{{ item.state_name }}</span>
          </td>
          <td class="caozuo_td">
          <!-- 不同管理员权限下的不同操作 -->
            <!-- 仅可见 -->
            ...
            <div v-else-if="item.can_upgrade && item.can_edit">
              <div v-if="item.can_upgrade" style="display: inline-block;padding-right:10%;" @click="handleCaozuo('部署', 'upgrade', item)"><span>部署</span></div>
              <div v-if="item.can_edit" style="display: inline-block;" @click="editrunning(item)"><span>查看与编辑</span></div>
              <div class="shenglue">
                <img src="../../img/shenglue.png" @click="clickShenglue('caozuo',item,index)">
              </div>
              <div v-if="item.isShowCaozuo" class="caozuo_items" >
                <div v-if="item.can_restart && item.state == 0" class="caozuo_item" @click="handleCaozuo('重启', 'restart', item)">重启</div>
                <div v-if="item.can_start && item.state == 3" class="caozuo_item" @click="handleCaozuo('启动', 'start', item)">启动</div>
                <div v-if="item.can_stop && item.state == 0" class="caozuo_item" @click="handleCaozuo('停止', 'stop', item)">停止</div>
            ...
            <!-- 基础操作 -->
            <div v-else>
              <div v-if="item.can_restart && item.state == 0" style="display: inline-block;padding-right:10%;" class="caozuo_item" @click="handleCaozuo('重启', 'restart', item)"><span>重启</span></div>
              <div v-if="item.can_start && item.state == 3" style="display: inline-block;padding-right:10%;" class="caozuo_item" @click="handleCaozuo('启动', 'start', item)"><span>启动</span></div>
      ...
      <div class="pagebar" v-if="!noData"><PagerBar :page="page" @change="searchByPage"></PagerBar></div>
    </div>
    <AddRunning 
      ref="addrunning" 
      :obj="editRunningObj" 
