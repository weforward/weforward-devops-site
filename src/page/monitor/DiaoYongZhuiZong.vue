    ...
    <div class="query_box">
      ...
      <div class="dierhang">
        <div class="choose_method">
          追踪ID：<input type="text" class="input" v-model="idValue">
        </div>
        <div style="margin-left: 1%" class="choose_method">
          方法名：<input type="text" class="input" v-model="methodValue">
        </div>
        <div class="choose_name">
          服务名：
          <SingleSelect :width='100' :items="serviceNameList" @select="changeName"
                        :selectname='serviceName'></SingleSelect>
          <span style="margin-left: 2%">服务编号：<SingleSelect :width='100' :items="serviceNoList" @select="changeNo"
...
export default {
  ...
  mounted() {
    ...
    //获取前五分钟时间段
    var time = new Date()
    this.dateTime = [new Date((time.getTime() - 60000)).Format(), this.Format()]
    this.query();
  }
}
</script>
