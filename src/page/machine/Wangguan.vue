<template>
  <div class="wangguan_area">
    ...
    <div class="facts_boxs">
      <div class="selbox">
        网关：
        <SingleSelect :items="wangguanList" @select="changeNo" :selectname='id'></SingleSelect>
        <div class="time_box">
          时间段：
          <el-date-picker
              v-model="dateTime"
        ...
        <el-button type="primary" size="small" class="ml-1" @click="freshEcharts">查 询</el-button>
        <el-button type="primary" size="small" class="ml-1" @click="freshEcharts">刷 新</el-button>
        <el-button style="width:80px;background-color: #563d7c;color:#ffe484;border-color: #563d7c" size="small"
                   class="ml-1"
                   @click="goToRlog">查看日志
        </el-button>
      </div>
      <div class="item_box">
        ...
        <div style="padding-left: 10px;padding-right: 10px;" :class="['query_button',{'is_sel':!isRecentHour }]"
             @click="clickTime('recentHour')">近24小时
        </div>
  ...
  methods: {
    freshEcharts() {
      ...
      if (!this.dateTime) {
        ...
        } else if (new Date(this.dateTime[1]) - new Date(this.dateTime[1]) <= 60000) {
    ...
    clickTime(op) {
      if (this.id) {
        ...
        if (op == 'onTime') {
          ...
          this.dateTime = [this.queryKsTime.Format('y-M-d h:m'), this.queryJsTime.Format('y-M-d h:m')]
          this.drawEcharts();
    ...
    async drawMemory() {
      ...
      async function newData() {
        ...
        let newfenpei = await that.$wf.post("devops?method=/devops/metrics/gw_memory_alloc_range", {
          end: newend,
          begin: newbegin,
          id: that.id
        })
        let newused = await that.$wf.post("devops?method=/devops/metrics/gw_memory_used_range", {
          end: newend,
          begin: newbegin,
          id: that.id
        })
