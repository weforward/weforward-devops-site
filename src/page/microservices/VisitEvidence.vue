<template>
  <div class="fwpz_area">
    <div class="query">
      ...
      <div class="input_box ml-2">关键字：<input type="text" v-model="keyword" class="input" @keydown.enter="query"></div>
      <el-button type="primary" size="small" class="ml-1" @click="query">查 询</el-button>
      <el-button type="primary" size="small" class="ml-1" @click="showcreate = !showcreate">+ 添 加</el-button>
    </div>
    <div>
    ...
    <WoolShowmodal :isshow="showcreate" @tapCloser="showcreate = !showcreate" :isshowcloser="true">
      <div class="add_fwpz">
        ...
        <div class="kind_items">
          ...
          <WoolSelect :items="cSelectOption" v-model="createOptionVal" class="kindselect">
            {{ cSelectOption[createOptionVal].name }}
          </WoolSelect>
        </div>
        <!--
        ...
        <div class="zhuaiyao_items">
          ...
          <textarea v-model="summary" class="summary-input zhaiyao" rows="5" style="width:550px"
                    placeholder="请输入凭证摘要"></textarea>
        </div>
        <div class="btn-box">
    ...
    <WoolShowmodal :isshow="showaccesskey" @tapCloser="showaccesskey = !showaccesskey" :isshowcloser="true">
      <div class="accesskey">
        ...
        <div><span class="accesskey-item">accessId = {{ accessId }}</span><img src="../../img/copy.png"
                                                                               @click="copy(accessId)"/></div>
        <div><span class="accesskey-item">accesskey = {{ accesskey }}</span><img src="../../img/copy.png"
                                                                                 @click="copy(accesskey)"/></div>
        <div><span class="accesskey-item">accessKey（base64） = {{ accesskey_base64 }}</span><img src="../../img/copy.png"
                                                                                 @click="copy(accesskey_base64)"/></div>
      </div>
    </WoolShowmodal>
  </div>
</template>
...
export default {
  ...
  methods: {
      ...
      this.$wf
          ...
          .then(data => {
            if (data.id) {
              ...
              this.accesskey_base64 = data.key_base64
    ...
    seeAccesskey(item) {
      ...
      this.accesskey_base64 = item.key_base64
    },
